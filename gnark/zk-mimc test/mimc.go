...
// m: message
// k: encryption key
func (d *digest) encrypt(m fr.Element, k fr.Element) fr.Element {
	once.Do(initConstants) // init constants
...
// func (d *digest) decrypt(h fr.element) fr.element {
// 	once.Do(initConstants) // init constants

// 	for i := mimcNbRounds - 1; i >= 0; i-- {
// 		// m = (m+k+c)^-5
// 		var tmp fr.element
// 		tmp.Add(&h, &c).Add(&tmp, &mimcConstants[i])
// 		h.Square(&tmp).
// 			Square(&h).
// 			Mul(&h, &tmp)
// 	}
...
func (d *digest) decrypt(m fr.Element) fr.Element {
	once.Do(initConstants) // as key

	for i := 0; i < mimcNbRounds; i++ {
		// m = (m+k+c)^-5 = Inv((m+k+c)^5)
		var tmp fr.Element
		tmp.Add(&m, &d.h).Add(&tmp, &mimcConstants[i])
		m.Square(&tmp).
			Square(&m).
